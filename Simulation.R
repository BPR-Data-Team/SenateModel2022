...
combination <- expand.grid(stateslist, stateslist) %>%
  ...
  mutate(correlation = case_when(
    r_value(state1, state2) < 0 ~ 0,
    TRUE ~ r_value(state1, state2)))
...
statessimulationresult <- simulation %>%
  ...
  mutate(chanceofwinning = case_when(
    chanceofwinning == 0 ~ 0.001,
    chanceofwinning == 1 | chanceofwinning == 1.0001 ~ 0.999,
    TRUE ~ chanceofwinning
  )) %>%
...
write_sheet(statessimulationresult, ss = as_sheets_id("https://docs.google.com/spreadsheets/d/1gXGn5vKjhYyhi2-38qiH4Ps5qMvppXg3JFSVYijZLFU/edit#gid=800772858"), sheet = 17)
...
write_sheet(as.data.frame(c(mean(listofresults) + 1.645 * sd(listofresults), mean(listofresults) - 1.645 * sd(listofresults))), ss = as_sheets_id("https://docs.google.com/spreadsheets/d/1gXGn5vKjhYyhi2-38qiH4Ps5qMvppXg3JFSVYijZLFU/edit#gid=800772858"), sheet = 19)
